...
def plot_value_function(actor, critic):
    fig = plt.figure()
    ...
    for i in range(M.shape[0]): 
        for j in range(M.shape[1]):
            ...
            action_value = critic.forward(state, actions)[0].cpu().detach().numpy()
            M[j,i] = action_value
...
def plot_policy(actor):
    ...
    for i in range(M.shape[0]): 
        for j in range(M.shape[1]):
            state = torch.tensor([np.array([0, y[i], 0, 0, w[j], 0, 0, 0])], dtype=torch.float32)
            actions = actor.forward(state)[0].cpu().detach().numpy()
            if actions[1] > 0.5:
                action_value = 1
            elif actions[1] < -0.5:
                action_value = -1
...
def plot_agent_rewards(actor):
    ...
    ax[1].plot([i for i in range(1, 51)], episode_reward_list, label='DDPG Agent episode reward')
    ax[1].set_xlabel('Episodes')
    ax[1].set_ylabel('Total number of steps')
    ax[1].set_ylim(-500,400)
    ax[1].set_title('DDPG Agent episode reward')
    ax[1].legend()
    ax[1].grid(alpha=0.3)
    plt.show()
    ...
    for i in range(50):
        ...
        while not done:
            ...
            if agent is None:
                ...
                action = actor.forward(torch.tensor([state], dtype=torch.float32))[0].cpu().detach().numpy()
...
def main():
    ...
    actor = torch.load(path.joinpath('neural-network-2-actor.pth'))
    critic = torch.load(path.joinpath('neural-network-2-critic.pth'))
    plot_value_function(actor, critic)
    plot_policy(actor)
